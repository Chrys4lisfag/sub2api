...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'launch-browser', 'create-spark-shadow'])
const { t } = useI18n()
...
const isAntigravityOAuth = computed(() => (props.account?.platform === 'antigravity' || props.account?.platform === 'antigravity_native') && props.account?.type === 'oauth')
