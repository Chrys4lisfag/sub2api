...
func ProvideConcurrencyCache(rdb *redis.Client, cfg *config.Config) service.ConcurrencyCache {
	waitTTL := cfg.Gateway.Scheduling.StickySessionWaitTimeout
	if cfg.Gateway.Scheduling.FallbackWaitTimeout > waitTTL {
		waitTTL = cfg.Gateway.Scheduling.FallbackWaitTimeout
	}
	waitTTLSeconds := int(math.Ceil(waitTTL.Seconds()))
	if waitTTLSeconds <= 0 {
